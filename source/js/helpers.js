...
hexo.extend.helper.register('lunr_index', function(data){
  ...
  _.sortBy(data, 'title').forEach((item, i) => {
...
hexo.extend.helper.register('get_manuals', function(){
  var name = this.page.name;
  var data = this.site.data[this.page.data];

  var target = _.find(data, function(value) {
    return value.name === name;
  });

  return _.get(target, 'manuals', []);
});

hexo.extend.helper.register('icon', function (icon, size) {
    return '<i class="fa fa-' + icon + (size ? ' fa-' + size + 'x' : '') + '"></i>'
});

hexo.extend.helper.register('return_href', function () {
    var path = this.page.path;
    var node = path.split('/');
    var url = node.slice(0, -2).join('/')
    return '/' + url;
});

hexo.extend.helper.register('time_from', function (time) {
